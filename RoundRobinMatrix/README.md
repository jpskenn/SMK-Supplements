...
- 使用できる条件が限られる  
電圧降下とダイオードの特性によって動作条件が決まるため、部品の組み合わせによっては動作しないことがあります。
現在のところ、Pro Microを含むATmega32u4と、1N4148ダイオードの組み合わせで動作することが確認できています。

- RemapやVIAで使用できるレイヤー数が減る場合がある  
COL2ROWのマトリクス処理にした場合、使用するピン数をMATRIX_ROWSとMATRIX_COLSへ重複して定義するため、EEPROMの使用サイズが多くなり、保存できるレイヤー数が減る場合があります。  
カスタムマトリクスによるスキャン処理をおこなえば回避可能です。
...
1. 使用するピンの数を決めます。
    ...
    ```aside
    75キーの場合は、次のような計算で、ピンの数を10個と決めます。
    9ピン ：9(9-1)=72キー → 足りない
    10ピン：10(10-1)=90キー → 足りる
    ```
...
の経路でPIN1の状態がPIN3で判定されます。

もし、この判定結果がLoになると、押していないスイッチSW13が押されていると認識され、ゴーストが発生します。

ATmega32u4が信号をLoとして判定する閾値のVIHminは、  
...
さて、条件として挙げた経路では、合計4個のダイオードを通過しており、ダイオード1個あたりの電圧降下を少なめの0.6Vと見積もった場合でも、合計2.4Vの電圧降下が発生します。  

この電圧降下によってATmega32u4のVIHmin 1.9Vを超えるため、PIN1はHiと判定され、ゴーストは発生しません。
...
**VIAを使用する場合（rules.mkで`VIA_ENABLE = yes`を指定した場合）は、Bootmagicが自動で有効になるため、指定し忘れに注意してください。**
