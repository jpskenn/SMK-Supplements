...
### rules.mk
  ...
  変更なし。  
  標準のCOL2ROWを使用するため、カスタムマトリクスの指定は不要です。
...
- ダイオードの方向  
  ...
  ROWとCOLに同じピン番号を指定しているので、COL2ROWを指定するだけで、すべてのピン同士でスイッチ押下のスキャンがおこなわれます。

- ブートローダへ入るためのキー指定
  Bootmagic Liteを使用する場合は、必要に応じて、左上キーのマトリクス上の位置指定します。\
  **VIAを使用する場合も同様です。**（VIA使用時は、Bootmagic Liteが内部的に自動で有効になるため。）\
  \
  何も指定しない場合、QMKのデフォルトでは、マトリクスの左上(0,0)のキーがブートローダへ入るために使用されます。\
  ...
  例：config.hでROW_PINSとCOL_PINSでピン番号の並びを入れ替え、keyboard.hでその並びに合わせたマトリクスを構成して回避する。
  ```
